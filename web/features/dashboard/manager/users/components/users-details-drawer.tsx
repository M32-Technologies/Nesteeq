...
export default function UsersDetailsDrawer({
  residentId,
  open,
  mode,
  onModeChange,
  ...
  const {
    ...
    isError,
    refetch,
  } = useResidentDetailsQuery(open ? residentId : null)
  const updateResident = useUpdateResidentMutation()
  const updateStatus = useUpdateResidentStatusMutation()
  const [statusAction, setStatusAction] = useState<ResidentStatus | null>(null)
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    ...
    reset({
      name: resident.name === "Unknown user" ? "" : resident.name,
  ...
  const submitEdit = async (values: ResidentFormValues) => {
  ...
  const handleStatusChange = async () => {
    ...
    const nextStatus: ResidentStatus =
      resident.status === "inactive" ? "active" : "inactive"
                ...
                <div className="space-y-5 p-5">
                  <section className="rounded-xl border border-slate-200 bg-white px-4 py-2">
                    ...
                    <DetailRow
                      icon={<Mail size={16} />}
                      label="Email"
                      value={<span className="block max-w-[210px] truncate">{resident.email}</span>}
                    />
                    <DetailRow
                  ...
                  <section className="rounded-xl border border-slate-200 bg-white px-4 py-2">
                    ...
                    <DetailRow
                      ...
                      label="User Type"
                      value={<ResidentTypeBadge type={resident.type} />}
                    />
                    <DetailRow
                    ...
                    <DetailRow
                      ...
                      label="Joined At"
                      value={formatDate(resident.joinedAt)}
                    />
                  </section>
                  ...
                  <section className="rounded-xl border border-slate-200 bg-white px-4 py-2">
                    ...
                    <DetailRow
                      icon={<CalendarDays size={16} />}
                      label="Created At"
                      value={formatDateTime(resident.createdAt)}
                    />
                    <DetailRow
                      icon={<RefreshCw size={16} />}
                      label="Last Updated"
                      value={formatDateTime(resident.updatedAt)}
                    />
                  </section>
                </div>
              </div>
            ...
            >
              {resident.status === "inactive" ? (
                <>
                  ...
                  {statusAction === "active" ? "Activating..." : "Activate User"}
                </>
              ) : (
                <>
                  <UserX size={16} />
                  {statusAction === "inactive" ? "Deactivating..." : "Deactivate User"}
                </>
              )}
            </button>
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-1 max-w-[260px] text-sm text-slate-500">
        Something went wrong while loading the resident details.
      </p>
...
function ResidentTypeBadge({ type }: { type: ResidentType }) {
  return (
