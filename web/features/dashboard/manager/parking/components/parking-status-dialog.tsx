...
type ParkingStatusDialogProps = {
  slot: ParkingSlot | null
  status: "AVAILABLE" | "INACTIVE" | null
  open?: boolean
  onClose: () => void
  onSuccess?: () => void
}

export function ParkingStatusDialog({
  slot,
  status,
  open: controlledOpen,
  onClose,
  onSuccess,
}: ParkingStatusDialogProps) {
  const updateStatus = useUpdateParkingStatusMutation()
  const [formError, setFormError] = useState("")

  const isOpen = controlledOpen !== undefined ? controlledOpen && Boolean(slot && status) : Boolean(slot && status)

  if (!isOpen || !slot || !status) {
    return null
  }

  const isActivating = status === "AVAILABLE"
  ...
  const submitStatusChange = async () => {
    try {
      ...
      toast.success(
        isActivating
          ? "Parking slot reactivated successfully"
      ...
      const message =
        error instanceof Error ? error.message : "Failed to update slot status"
      toast.error(message)
        ...
        <div className="flex min-h-[82px] items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm font-semibold leading-5 text-slate-500">
              {isActivating
                ? "This slot will move back to available slots."
                : "This action does not permanently delete the slot."}
            </p>
          </div>
          ...
          <button
            ...
            aria-label={`Close ${actionLabel.toLowerCase()} slot dialog`}
          >
            <X size={19} />
          </button>
        </div>
            ...
            <div>
              ...
              >
                {isActivating
                  ...
                  : "The parking slot stays in records, but active parking workflows should no longer use it."}
              </p>
            </div>
          </div>
        </div>
