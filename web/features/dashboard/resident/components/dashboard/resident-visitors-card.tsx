...
export function ResidentVisitorsCard({
  ...
  const displayItems = guestPasses.slice(0, 3).map((pass, index) => ({
    ...
    vehicle: pass.vehicleNumber || null,
    passCode: `#QR-${pass._id.slice(-4).toUpperCase()}`,
    isDelivery: pass.purpose?.toLowerCase().includes("delivery") || false,
    ...
    if (navigator.share) {
      navigator
        .share({
          ...
          text: `Here is your visitor gate pass: ${code}. Present this at the society security gate.`,
        })
        .catch(() => {});
    } else {
    ...
    <div className="rounded-2xl border border-slate-200/90 bg-white p-5 sm:p-6 shadow-[0_2px_8px_rgba(15,23,42,0.03)] space-y-4">
      ...
      {displayItems.length > 0 ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-200 p-6 text-center space-y-2">
          <Users className="size-6 text-slate-400 mx-auto" />
          <p className="text-xs font-bold text-slate-700">No upcoming visitors expected</p>
          <p className="text-[11px] text-slate-400">
            Pre-approve visitor passes or food delivery gate entries anytime.
          </p>
        </div>
      )}
    </div>
